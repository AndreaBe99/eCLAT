...
class Program(BaseBox):
    ...
    def exec(self, env):
        ...
        # registry.csv in runtime for eclat module 
...
class FromImport(BaseBox):
    ...
    def prima_passata(self, env):
        # Controllo il modulo
        if self.to_co == "net":
            path = settings.ECLAT_DATA
            for statement in self.args.get_statements():
                #currentDirectory = os.getcwd()
                #os.chdir(path)
                path = (path + self.to_co).replace("/", ".")
                try:    
                    Appoggio.import_module[str(statement)] = importlib.import_module(path, statement)
                    #Appoggio.import_module[str(statement)] = __import__(self.to_co, str(statement))
                except:
