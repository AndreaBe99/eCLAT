            ...
            for var in json_dict[obj]["vars"]:
                ...
                json_output.append({
                    "program": {
                        ...
                        "members": read_members(struct["members"][0])
                    },
                    "map": {
                        "name": struct["members"][1]["name"],
                        "members":read_members(struct["members"][1])
                    }
                })
    ...
    if not "members" in json_obj and not "type_id" in json_obj:
        if json_obj["kind"] == "FUNC_PROTO":
            ...
            for param in json_obj["params"]:
                ...
                params.append(read_members(json_dict[param["type_id"]]))
            return {
                'kind': json_obj["kind"],
                'name': json_obj["name"],
                'return': read_members(json_dict[json_obj["ret_type_id"]]),
                'params': params
            }
        else:
            return {
        ...
        for member in json_obj["members"]:
            ...
            # Se è un tipo semplice
                ...
                members.append({
                    "name": member["name"],
                    "type": read_members(member)
                })
        return members

    elif "type_id" in json_obj:
        return read_members(json_dict[json_obj["type_id"]])
