...
class Program(BaseBox):
    ...
    def eval(self, env):
        # ----------------------------------------- #
        # SALVO I PROGRAMMI DI                      #
        # eclat_program_list.csv IN UN DICT         #
        # PER COMODITA'                             #
...
class FromImport(BaseBox):
    ...
    def to_c(self):
        ...
        for statement in self.args.get_statements():
            # ------------------------- #
            # Se c'è l'import di Packet #
            #  asseggno le sue funzioni #
            #  ad un dict.              #
            # ------------------------- #
...
class FunctionDeclaration(BaseBox):
    ...
    def to_c(self):
        ...
        if len(Appoggio.funzioni_variabili_locali[self.name]) != 0:
            res += Appoggio.indent_level*"\t" + "__u64 " \
                + ', '.join(Appoggio.funzioni_variabili_locali[self.name]) + ";"
    ...
    def to_c(self):
        ...
        if type(self.right) is Call:
            ...
            if self.right.name[:11] == "Packet.read":
                if not self.right.name in Appoggio.funzioni.keys():
                    raise Exception("Packet Class does not exist")
