...
class Parser():
    ...
    def parse(self):
        ...
        @self.pg.production('statement : DEF IDENTIFIER ( arglist ) : NEWLINE block END')
        def statement_func(p):
            return FunctionDeclaration(p[1].getstr(), Array(p[3]), p[7])

        @self.pg.production('statement : DEF IDENTIFIER ( ) : NEWLINE block END')
        def statement_func_noargs(p):
            return FunctionDeclaration(p[1].getstr(), Null(), p[6])
        
        @self.pg.production('expression : IF expression : statement END')
        def expression_if_single_line(p):
            return If(condition=p[1], body=p[3])

        @self.pg.production('expression : IF expression : statement ELSE : statement END')
        def expression_if_else_single_line(p):
            return If(condition=p[1], body=p[3], else_body=p[6])

        @self.pg.production('expression : IF expression : NEWLINE block END')
        def expression_if(p):
            return If(condition=p[1], body=p[4])

        @self.pg.production('expression : IF expression : NEWLINE block ELSE : NEWLINE block END')
        def expression_if_else(p):
            return If(condition=p[1], body=p[4], else_body=p[8])
        
        @self.pg.production('expression : WHILE expression : NEWLINE block END')
