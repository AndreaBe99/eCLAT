...
class Appoggio():    
    ...
    funzioni = {}                   # dict provvisorio per la funzione Packet
...
def find_Program(statement):
    if statement in Appoggio.funzioni_eclat:
        return str(Appoggio.funzioni_eclat[statement][0]) \
            + " " + str(Appoggio.funzioni_eclat[statement][1])
    if statement in Appoggio.funzioni_registry:
        return "HIKE_CHAIN_" \
            + str(Appoggio.funzioni_registry[statement][0]) + "_ID"
...
class Program(BaseBox):
    ...
    def eval(self, env):
        ...
        print("'eclat_output_test.c' was generated.")
        f = open("eclat_output_test.c", "w")
        f.write(output)
        f.close()
    ...
    def to_c(self):
        ...
        for statement in self.args.get_statements():
            if statement[:6] != "Packet":
                result += '#define ' + Appoggio.funzioni_eclat[statement][0] + " " \
                + Appoggio.funzioni_eclat[statement][1] + '\n'
        return result
...
class FunctionDeclaration(BaseBox):
    ...
    def prima_passata(self, env):
        ...
        # ----------------------------------------- #
        if self.name in Appoggio.funzioni_variabili_locali or self.name in Appoggio.funzioni:
    ...
    def to_c(self):
        ...
        if type(self.right) is Call:
            ...
            if self.right.name[:11] == "Packet.read":
                return Appoggio.funzioni[self.right.name] + "&" \
        ...
        if Appoggio.funzione_corrente != "":
            ...
            if not self.left.getname() in Appoggio.funzioni_variabili_locali[Appoggio.funzione_corrente] and \
                ...
                if type(self.right) is Call:
                    ...
                    if int(self.right.to_c()) < 2**8 -1:
                        Appoggio.tipo_variabili_locali[Appoggio.funzione_corrente].append((self.left.getname(), 8))
                    if 2**8 - 1 < int(self.right.to_c()) < 2**16 - 1:
                        Appoggio.tipo_variabili_locali[Appoggio.funzione_corrente].append((self.left.getname(), 16))
                    if 2**16 - 1 < int(self.right.to_c()) < 2**32 - 1:
                        Appoggio.tipo_variabili_locali[Appoggio.funzione_corrente].append((self.left.getname(), 32))
                    if 2**32 - 1 < int(self.right.to_c()) < 2**64 - 1:
                        Appoggio.tipo_variabili_locali[Appoggio.funzione_corrente].append((self.left.getname(), 64))
                elif type(self.right) is BinaryOperation:
                    if self.right.eval(env) < 2**8 - 1:
                        Appoggio.tipo_variabili_locali[Appoggio.funzione_corrente].append((self.left.getname(), 8))
                    if 2**8 - 1 < self.right.eval(env) < 2**16 - 1:
                        Appoggio.tipo_variabili_locali[Appoggio.funzione_corrente].append((self.left.getname(), 16))
                    if 2**16 - 1 < self.right.eval(env) < 2**32 - 1:
                        Appoggio.tipo_variabili_locali[Appoggio.funzione_corrente].append((self.left.getname(), 32))
                    if 2**32 - 1 < self.right.eval(env) < 2**64 - 1:
                        Appoggio.tipo_variabili_locali[Appoggio.funzione_corrente].append((self.left.getname(), 64))
                else:
                    Appoggio.tipo_variabili_locali[Appoggio.funzione_corrente].append((self.left.getname(), 64))
...
class Call(BaseBox):
    ...
    def to_c(self):
        # -------------------------------------------#
        # CONTROLLO ERRORE SULLA CALL                #
        # -------------------------------------------#
        if not self.name in Appoggio.funzioni_eclat and \
            not self.name in Appoggio.funzioni_registry and \
                not self.name in Appoggio.funzioni_alias and \
                    not self.name in Appoggio.funzioni.keys():
        ...
        if self.name in Appoggio.funzioni:
            ...
            # il primo parametro va assegnato mentre    #
            # il secondo va tra parentesi               #
            # ----------------------------------------- #
            ...
            # .right dell'Assignment sotto forma di     #
        ...
        if self.name in Appoggio.funzioni_eclat:
            ...
            return 'hike_elem_call_' + str(param_number+1) \
                + '(' + Appoggio.funzioni_eclat[self.name][0] + parametri
